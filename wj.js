...
function WebRTC(options) {
    ...
    $(this.divReconnectButton).click(function () {
        ...
        $("#divDurationCallAudio").html('<audio src="/Content/wechat_video_sounds.mp3" autoplay loop></audio>');
    ...
    this.waitForOffer = function () {
        if (that.tryConnectTimes >= 10) {
            ...
            toastr.warning("Your call cannot be connected at this time, please try again later<br/>\
                <a href='/User/Dashboard' class='btn btn-primary'>OK</a>", "Message", { timeOut: 300000, positionClass: 'toast-top-right', tapToDismiss: false });
            return;
        }
    ...
    this.checkRemoteStream = function (remoteEvent) {
        if (remoteEvent) {            
            ...
            if (!navigator.mozGetUserMedia)
                that.remoteVideo.src = that.URL.createObjectURL(remoteEvent.stream);
            else
                that.remoteVideo.mozSrcObject = remoteEvent.stream;
    ...
    this.waitForParticipant = function () {        
        if (that.tryConnectTimes >= 10) {
            ...
            $(that.divNoResponsePanel).show();
            $("#divDurationCallAudio").html("");
            toastr.warning("The user your calling is not available");
            return;
        }
        that.tryConnectTimes++;
    ...
    this.onMessage = function (event) {
        ...
        if (option.type == "text" || option.type == "file") {
            ...
            //popup tip message
            if ($("#divCanvasPanel").hasClass("col-md-9"))
        ...
        if (option.type == "text") {
            ...
            if (option.key2 == "Direct") {     
                ...
                ctx.drawImage(document.getElementById("imgCanvasAreaImageData"), 0, 0, $(canvas).width(), $(canvas).height());
            }
            else {
            ...
            iframeCanvasArea.setPdfImageList(option.key2, option.opt.Width, option.opt.Height);
            return;
        }
        else if (option.type == "SelectBindChange") {
            $("#selectPdfImageList").val(option.key1);
            iframeCanvasArea.changePdfImageIndex(option.key2);
        }
